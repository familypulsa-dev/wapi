...
func (c *CloudClient) SendMarketing(ctx context.Context, phoneNumberID string, msg *types.Message) (*types.SendResponse, error) {
	msg.MessagingProduct = "whatsapp"
...
func (c *CloudClient) SendTyping(ctx context.Context, phoneNumberID, messageID string) error {
	msg := types.NewTypingIndicator(messageID)
	path := fmt.Sprintf("%s/messages", phoneNumberID)
	return c.do(ctx, "POST", path, msg, nil)
}
